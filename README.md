...
识别结束后的 8 秒是软超时：如果豆包仍在提交 marked text，窗口会提示“识别仍在完成，可等待或使用下方按钮”并继续轮询；最多等待 30 秒才触发超时。窗口底部的“复制文字”会保留文字并写入本机剪贴板，“同步到远端”与 Return 使用相同发送流程。

同步前会短暂重试 UU 窗口激活，以应对 bridge 自身暂时处于前台的情况；第三方应用抢占焦点或超过重试期限则取消发送。

获得 UU 焦点后还会等待约 0.45 秒并再次确认剪贴板与窗口焦点，再发送一次 Command-V，避免焦点刚切换完成时过早发送。

在 IOHID fallback 路径中，首次右 `Option` 由物理按键直接唤醒豆包，桥接层不再合成启动/停止按键；首次松开仅记录状态，第二次松开再完成发送。
